...
class CategoryController extends Controller
{
    ...
    public function Delete_Every_Materials(Category $category) //Удаление материалов с этой категорией
    {
        ...
        foreach($materials as $material)
        {
            ...
            foreach($tags_material as $tags_material) //Удаление всех тегов данного материала
            {
                $tags_material->delete();
            }

            $links = Links::select()->where('fk_id_material', $material->id_material)->get();
            foreach($links as $link)//Удаление ссылков данного материала
            {
                $link->delete();
            }
