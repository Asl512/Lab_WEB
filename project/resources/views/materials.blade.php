                ...
                @else
                    ...
                    <tbody>
                        @foreach($materials as $material)
                            <tr>
                                <td><a href="{{ url('view-material?id='.$material->id) }}">{{$material->name}}</a></td>
                                <td>{{$material->autors}}</td>
                                <td>{{$material->fk_id_type}}</td>
                                <td>{{$material->fk_id_category}}</td>
                                <td class="text-nowrap text-end">
                                    <a href="create-material?id={{$material->id}}" class="text-decoration-none me-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"class="bi bi-pencil" viewBox="0 0 16 16">
                                            <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
                                        </svg>
                                    </a>
                                    <a data-bs-toggle="modal" href="#exampleModalToggle{{$material->id}}" role="button" class="text-decoration-none">
...
@section('end')
    @foreach($materials as $material)
        <div class="modal fade" id="exampleModalToggle{{$material->id}}" aria-hidden="true" aria-labelledby="exampleModalToggleLabel"tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form action="{{ route('DeleteM',$material->id) }}" method="POST">
