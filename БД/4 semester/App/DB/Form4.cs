...
namespace DB
{
    public partial class Form4 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = String.Format("SELECT name, surname FROM students WHERE id IN(SELECT students_exam.fk_id_student  FROM students, students_exam  WHERE students.id = students_exam.fk_id_student GROUP BY students_exam.fk_id_student HAVING SUM(students_exam.appraisal) / COUNT(students_exam.appraisal) = '{0}')",comboBox1.Text);
            connect.Open();
            cmd.ExecuteNonQuery();
            connect.Close();
